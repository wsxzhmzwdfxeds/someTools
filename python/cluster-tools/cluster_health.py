...
image = "127.0.0.1:29006/library/memcached:check3"
...
def component_status():
    status = k8sclient.cluster_health()
    dic = {}
    err_list = list()
    err_list.append('unhealthy components: ')
    ok = True
    for name, condition in zip((n.metadata.name for n in status), (n.conditions[0].message for n in status)):
        dic[name] = condition
        if condition.find('connection refused') != -1:
